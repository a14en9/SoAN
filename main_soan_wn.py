...
def _init_():
    ...
    os.system('cp models/SoAN_wn.py outputs' + '/' + args.exp_name + '/' + 'SoAN_wn.py.backup')
...
def train(args, io):
    ...
    for epoch in range(args.epochs):
        ...
        outstr = 'Train Epoch %d, Loss: %.6f, Train Glacial Lake IoU: %.6f, Train Background IoU: %.6f' % (
        epoch, loss_d['train'], train_metrics['IoU'][0], train_metrics['IoU'][1])
        io.cprint(outstr)
        # Validation loop
        ...
        outstr = 'Val   Epoch %d, Loss: %.6f, Val   Glacial Lake IoU: %.6f, Val   Background IoU: %.6f' % (
        epoch, loss_d['val'], val_metrics['IoU'][0], val_metrics['IoU'][1])
        io.cprint(outstr)
        # Save model
...
if __name__ == '__main__':
    ...
    parser.add_argument("--saved_model_dir", type=str, default="outputs/SoAN_wn/models/model_optimal.pt")
    parser.add_argument("--train_yaml", type=str, default="conf/train_soan_wn.yaml")
    parser.add_argument("--batch_size", type=int, default=32)
    parser.add_argument("--exp_name", type=str, default="SoAN_wn")
