...
def log_images(writer, frame, batch, epoch, stage="train"):
    ...
    ####  summary images in grid
    # if epoch == 0:

    if stage == 'train':
        writer.add_image(f"{stage}/x", make_grid(pm(squash(x[:, :, :, :3])), scale_each=True), epoch)
        writer.add_image(f"{stage}/y", make_grid(y.unsqueeze(1), scale_each=True), epoch)
        writer.add_image(f"{stage}/y_hat", make_grid(y_hat.unsqueeze(1), scale_each=True), epoch)
    elif stage == 'val':
        writer.add_image(f"{stage}/x", make_grid(pm(squash(x[:, :, :, :3])), scale_each=True), epoch)
        writer.add_image(f"{stage}/y", make_grid(y.unsqueeze(1), scale_each=True), epoch)
        writer.add_image(f"{stage}/y_hat", make_grid(y_hat.unsqueeze(1), scale_each=True), epoch)
    else:
        writer.add_image(f"{stage}/x", make_grid(pm(torch.from_numpy(
            squash(np.squeeze(np.stack((x[:, :, :, 3:4], x[:, :, :, 2:3], x[:, :, :, 1:2]), axis=3), axis=4)))),
                                                 scale_each=True), epoch)
        writer.add_image(f"{stage}/y", make_grid(y.unsqueeze(1), scale_each=True), epoch)
        writer.add_image(f"{stage}/xy", make_grid(pm(np.multiply(torch.from_numpy(
            squash(np.squeeze(np.stack((x[:, :, :, 3:4], x[:, :, :, 2:3], x[:, :, :, 1:2]), axis=3), axis=4))),
                                                                 mp(y.unsqueeze(1)[:, :, :, :256]))), scale_each=True),
                         epoch)
        writer.add_image(f"{stage}/y_hat", make_grid(y_hat.unsqueeze(1), scale_each=True), epoch)
        writer.add_image(f"{stage}/xy_hat", make_grid(pm(np.multiply(torch.from_numpy(
            squash(np.squeeze(np.stack((x[:, :, :, 3:4], x[:, :, :, 2:3], x[:, :, :, 1:2]), axis=3), axis=4))),
                                                                     mp(y_hat.unsqueeze(1)[:, :, :,
                                                                        :256].cuda().cpu()))), scale_each=True), epoch)
