...
class RFB_modified(nn.Module):
    def __init__(self, in_channel, out_channel):
        ...
        # self.soan = nn.Sequential(NONLocalBlock2D(out_channel, sub_sample=True, bn_layer=True))
    ...
    def forward(self, x):
        # soan = self.soan(x)
        # x0 = self.branch0(soan)
        # x1 = self.branch1(soan)
        # x2 = self.branch2(soan)
        # x3 = self.branch3(soan)
        # x_cat = self.conv_cat(torch.cat((x0, x1, x2, x3), 1))
        #
        # x = self.relu(x_cat + self.conv_res(soan))
        # return x
        #
        x0 = self.branch0(x)
        x1 = self.branch1(x)
        ...
        x_cat = self.conv_cat(torch.cat((x0, x1, x2, x3), 1))
        # x = self.relu(x_cat + self.conv_res(x))
        x = self.relu(self.soan(x_cat) + self.conv_res(x))
        return x
    """
...
class aggregation(nn.Module):
    ...
    def __init__(self, channel):
        ...
        self.conv5 = nn.Conv2d(3 * channel, channel//2, 1)
...
class HarDMSEG(nn.Module):
    ...
    def __init__(self, inchannels = 15, outchannels=3, channel=32):
        ...
        self.upsample = nn.Upsample(scale_factor=8, mode='bicubic', align_corners=True)
        """
        self.hardnet = hardnet(arch=68, inchannels = inchannels, pretrained=False)
        # self.soan = nn.Sequential(NONLocalBlock2D(out_channel, sub_sample=True, bn_layer=True))
        self.seg_layer = nn.Conv2d(channel//2, outchannels, kernel_size=1)
    def forward(self, x):
        # print("input",x.size())
        ...
        ra5_feat = self.seg_layer(self.agg1(x4_rfb, x3_rfb, x2_rfb))

        # lateral_map_5 = self.upsample(ra5_feat)
...
if __name__ == '__main__':
    ras = HarDMSEG().cuda()
    input_tensor = torch.randn(1, 15, 256, 256).cuda()
