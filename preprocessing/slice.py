...
def slice_tile(img, size=(256, 256), overlap=6):
...
def slices_metadata(imgf, img_path, mask_path, size=(256, 256), overlap=6):
    """
    Write geometry and source information to metadata
    """
    meta = slice_polys(imgf, size, overlap)
    meta["img_source"] = str(img_path)
    meta["mask_source"] = str(mask_path)
    return meta


def slice_polys(imgf, size=(256, 256), overlap=6):
    """
...
def write_pair_slices(idx, img_path, mask_path, out_dir, border_path='',
    ...
    for k in tqdm(range(len(img_slices))):
        img_slice_path = Path(out_dir, f"{out_base}_{idx}_img_{k:03}.npy")
        mask_slice_path = Path(out_dir, f"{out_base}_{idx}_mask_{k:03}.npy")
...
if __name__ == '__main__':
    ...
    img_dir = Path(data_dir / "raster_data")
    mask_dir = Path(data_dir / "masks")
    out_dir = Path(data_dir / "slices")
    ...
    for i, name in enumerate(content):
        # img_dirs.append(str(Path(img_dir / name)))
        # mask_dirs.append(str(Path(mask_dir / name)))
        meta = write_pair_slices(i, Path(img_dir / name), Path(mask_dir / "mask_"f"{i:02}.npy"), out_dir)
        metadata.append(meta)
    metadata = pd.concat(metadata, axis=0)
    out_path = str(Path(out_dir, "slices.geojson"))
